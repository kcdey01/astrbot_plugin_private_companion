...
- 若astrbot版本号不为4.25.2，可在主动消息专用模式配置下关闭主链会话锁兼容模式。会话锁旨在解决这一版本astrbot的并发报错问题，但是会拖慢回复速度。
...
- 标签链路：插件提示词会优先要求 LLM 使用 `<pc_tts>...</pc_tts>`，发送前再交给插件统一转成语音组件。快速标签规则会按目标语种生成示例：中文语音直接中文朗读，日语/英语语音块后补对应中文含义；若当前 TTS provider 支持 `[happy]`、`[sad]` 这类情绪标签，规则和示例才会注入该条。这样可以避免 AstrBot 原生 `<tts>` 装饰器抢先处理，导致插件的语种修正、翻译和中文释义补全失效；历史或模型自行输出的 `<tts>` 仍会尽量兼容处理。
