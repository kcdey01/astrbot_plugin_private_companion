...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：5.3.1 继续收紧自然语言生图误触边界，并补齐夹层密码注入排障日志；默认行为保持稳，公开回复和主动图片能力仍建议按需开启。
...
- 相关设置：主动图片分享、生图后端、文生图工作流、自拍工作流、本地人设参考图、每日穿搭照片、本地图片读取保护。
...
- 使用自拍/人像链路，会复用本地人设参考图和在线图片 API 参考图能力；生成结果保存到插件数据中，并在拓展页左上角优先显示。
