...
- AstrBot 版本：`>=4.22.0`
- 编码要求：UTF-8
- 维护状态：本插件已完成缓存友好+功能精修重构，被动回复注入优化至4.0.0版本的1/3，缓存命中率由1/10优化至1/2+。后续版本将主要打磨使用体验，可能会重构主动消息链路。
- 若 AstrBot 版本号不为 4.25.2，可在拓展页-配置-功能开关下关闭主链会话锁兼容模式；会话锁旨在解决这一版本 AstrBot 的并发报错问题，但是会拖慢回复速度。
- 若好的想法仍可联系我，QQ：995051631（代码白痴），欢迎提交 Issues，目标是无愧“最强”之名，喜欢的话请点一个 Star。
